# 🎓 EduPro — Student Segmentation & Personalised Course Recommendation System# 🎓 EduPro — Student Segmentation & Personalised Course Recommendation System
...
**File:** `EduProOnlinePlatform.xlsx`

The workbook contains 4 sheets:

| Sheet | Rows | Description |
|---|---|---|
| `Users` | 3,000 | Learner demographics (UserID, UserName, Age, Gender, Email) |
| `Teachers` | 60 | Course instructor details (TeacherID, Name, Age, Gender, Expertise, YearsOfExperience, TeacherRating) |
| `Courses` | 60 | Course catalogue (CourseID, CourseName, CourseCategory, CourseType, CourseLevel, CoursePrice, CourseDuration, CourseRating) |
| `Transactions` | 10,000 | Purchase records linking Users, Courses, and Teachers (TransactionID, UserID, CourseID, TransactionDate, Amount, PaymentMethod, TeacherID) |

Full field-by-field definitions are in [`data_dictionary%20(2).md`](data_dictionary%20(2).md).
...
| `EduProOnlinePlatform.xlsx` | ✅ Required | Your dataset. It must be committed to the repo so the deployed app can read it. |
...
2. Create `requirements.txt` with:
   ...
   openpyxl
   ```
3. Push `streamlit_app.py`, `requirements.txt`, and `EduProOnlinePlatform.xlsx` to a GitHub repository.
...
Full field-by-field definitions are in [`data_dictionary.md`](data_dictionary.md).
